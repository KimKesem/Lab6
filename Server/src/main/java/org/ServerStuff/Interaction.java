...
public class Interaction {
    ...
    public Interaction() throws SocketException {
    }

    public static void interactiveModeOn(HashMap<String, Comandable> commands, MapWrapper<Integer, Person> models) throws Exception {
        while (mode) {
    ...
    public static void runCommand(HashMap<String, Comandable> commands, MapWrapper<Integer, Person> models) throws Exception {
        ...
        CommandResult res;
        if (!(commands.get(request.getCommand()) instanceof DataStore || commands.get(request.getCommand()) instanceof ClientThing)){
            if ((request.getArg() == null) && ((commands.get(request.getCommand()) instanceof IntArgument) || (commands.get(request.getCommand()) instanceof StringArgument))) {
                res = new CommandResult("No argument.", false);
                ...
                if (commands.get(request.getCommand()) instanceof IntArgument) {
                    try {
                        ...
                        res = new CommandResult("Argument's wrong data type.", false);
                    }
                } else {
                    res = commands.get(request.getCommand()).execute(request.getArg());
                }
    ...
    public static void runScript(HashMap<String, Comandable> commands, MapWrapper<Integer, Person> models, String[] arrayOfInput) throws IOException {
        CommandResult res;
        try {
